...
<div class="container">
    <!-- Sign up form -->
    <form action="{{ route('garson.update', $garson->id) }}" method="post" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <!-- Sign up card -->
    ...
    $(document).ready(function() {
        ...
        $.getJSON("https://api.telegram.org/bot6772062485:AAFM64_NymKrlBJ2TAKyUdu82fDq6jOch0M/getUpdates",
                function(data) {
                    ...
                    let veri = [];
                    $('.telegram').append(
                        "<option value='{{ $garson->garson_id }}'>{{ $garson->garson_adi }}</option>");
                    $('.garson_adi').val("{{ $garson->garson_adi }}");
        ...
        var masa_id = "{{ $garson->masa_id }}";
        var masalar = masa_id.split('-');

        masalar.forEach(function(value) {
